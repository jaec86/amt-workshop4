...
let clothY = 30
let clothX = 60
let spacing = Math.floor(canvas.width * 0.6 / clothX)
let tearDist = 60
let friction = 0.99
let bounce = 0.5

console.log(Math.floor(canvas.width * 0.6 / clothX))
...
class Cloth {
  constructor () {
    ...
    let startY = canvas.height / 2 - clothY * spacing / 2

    for (let y = 0; y <= clothY; y++) {
      for (let x = 0; x <= clothX; x++) {
        let point = new Point(startX + x * spacing, startY - 72 + y * spacing)
        if (y === 0) {
